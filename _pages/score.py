...
def calculate_hausdorff_distance(segmentation1, segmentation2):
    """Calculate Hausdorff distance between two segmentations."""
    u = np.array(segmentation1)
    v = np.array(segmentation2)
...
def score_group(group, label_dir_path, debug):
        ...
        for labelled_object in golden_data:
            ...
            if user_object and iou > 0.5:
                ...
                if "segmentation" in labelled_object and labelled_object["segmentation"] and "segmentation" in user_object and user_object["segmentation"]:
                    # Transform segmentation data to a format suitable for Hausdorff distance calculation
                    segmentation1 = [np.array(pts).reshape((-1, 2)) for pts in labelled_object["segmentation"]]
                    segmentation2 = [np.array(pts).reshape((-1, 2)) for pts in user_object["segmentation"]]
                    
                    if segmentation1 and segmentation2:
                        distance = calculate_hausdorff_distance(segmentation1[0], segmentation2[0])
    ...
    if hausdorff_distances:
        average_distance = sum(hausdorff_distances) / len(hausdorff_distances)
